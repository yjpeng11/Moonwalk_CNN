...
if __name__ == '__main__':
    ...
    clip = 0
    if os.path.exists(WORKSPACE + '/indivi_feat_train_flow/')==0:
        os.makedirs(WORKSPACE + '/indivi_feat_train_flow/')
        os.makedirs(WORKSPACE + '/indivi_feat_test_flow/')
    for videoid in range(max(vididx_train)+1):
        ...
        for clip in index:
            output_train = model.predict_generator(generator=getBatchGenerator(X_u_train[clip],X_v_train[clip], Y_train[clip], args, is_shuffle=False), steps=1)
        ...
        np.save(WORKSPACE + '/indivi_feat_train_flow/feat_train_video_' + str(videoid).zfill(5) + '.npy',CNN_features_train)
    ...
    for videoid in range(max(vididx_val)+1):
        ...
        for clip in index:
            output_val = model.predict_generator(generator=getBatchGenerator(X_u_val[clip], X_v_val[clip], Y_val[clip], args, is_shuffle=False), steps=1)
            CNN_features_val1 = np.squeeze(output_val[1])
            CNN_features_val.append(CNN_features_val1)
        np.save(WORKSPACE + '/indivi_feat_test_flow/feat_test_video_' + str(videoid).zfill(5) + '.npy', CNN_features_val)
