...
def build_model(args, weights_path_LSTM=None, output_feature = False):
    ...
    if args.returnseq:
        ...
        sequenceout = False
    model = Sequential()
    # X_image = Input(shape=(args.max_len,4096), name='input_image')
    # X_flow = Input(shape=(args.max_len,4096), name='input_flow')
    # video = concatenate([X_image, X_flow], axis=-1)
    video = Input(shape=(args.max_len, 4096))

    if sequenceout:
        model_seq = TimeDistributed(model, name='td_0')(video)
        encoder = LSTM(args.nodes, trainable=True, return_sequences=True, name='lstm_1')(model_seq)
        dropout = TimeDistributed(Dropout(0.75, name='dp_1'), name='td_1')(encoder)
        # output = TimeDistributed(Dense(args.num_classes, activation='softmax', init=my_init, name='dense_6'), name='td_2')(dropout)
        output = TimeDistributed(Dense(args.num_classes, trainable=True, activation='softmax', init=my_init, name='new_dense_6'), name='td_2')(dropout)
    else:
        model_seq = TimeDistributed(model, name='td_0')(video)
        encoder2 = LSTM(args.nodes, trainable=True, return_sequences=False, name='lstm_2')(model_seq)
        dropout = Dropout(0.75, name='dp_1')(encoder2)
        output = Dense(args.num_classes, trainable=True, activation='softmax', init=my_init, name='dense_6')(dropout)


    # model = Model(input=video, output=output)
    if output_feature:
        model = Model(inputs=video, outputs=[output, encoder2])
    else:
        model = Model(inputs=video, output=output)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_len', type=int, default=6, help='Max length')
    ...
    train_feature_path_fusion = os.path.join(WORKSPACE, 'fusion_train_data_feature.pik')
    train_feature_all_fusion = cPickle.load(open(train_feature_path_fusion))
    train_feature = train_feature_all_fusion['feature']

    val_data_Y_path = os.path.join(WORKSPACE, 'flow_test_data_Y.pik')
    Y_val = cPickle.load(open(val_data_Y_path))
    Y_val = Y_val['labels1']

    test_feature_path_fusion = os.path.join(WORKSPACE, 'fusion_test_data_feature.pik')
    test_feature_all_fusion = cPickle.load(open(test_feature_path_fusion))
    test_feature = test_feature_all_fusion['feature']
    ...
    # set up callbacks
    CHECKPOINT_DIR = WORKSPACE + '/checkpoints_fusion_LSTM/'
    ...
    if args.mode == 0: # training
        ...
        model.fit_generator(generator=getBatchGenerator(train_feature, Y_train, args),
                            steps_per_epoch = steps_train, epochs = args.num_epochs,
                            validation_data = getBatchGenerator(
                                test_feature, Y_val, args, is_shuffle = False),
        ...
        print 'ConfusionMat:', ConfusionMat
        print 'score:', score
        sio.savemat(WORKSPACE + '/feature_LSTM_fusion_val', {'LSTM_fea': LSTM_fea})
