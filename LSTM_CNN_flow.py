...
def build_model(args, weights_path_LSTM=None):
    """build flow model"""
    if args.returnseq:
        sequenceout = True
    else:
    ...
    if sequenceout:
        ...
        output = TimeDistributed(
            Dense(args.num_classes, trainable=True, activation='softmax', init=my_init, name='new_dense_6'),
            name='td_2')(dropout)
    else:
        model_seq = TimeDistributed(model, name='td_0')(video)
        # encoder = LSTM(args.nodes, return_sequences=True, name='lstm_1')(model_seq)
...
if __name__ == '__main__':
    ...
    train_feature_path = os.path.join(WORKSPACE, 'flow_train_data_feature_flow.pik')
    ...
    test_feature_path = os.path.join(WORKSPACE, 'flow_test_data_feature_flow.pik')
